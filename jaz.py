...
class DictationThread(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            # Use the main app's speak function if possible
...
class VoiceThread(QThread):
    ...
    def run(self):
        ...
        if app:
            for widget in app.topLevelWidgets():
                if isinstance(widget, CS2PApp):
                    widget.speak("Say C2 to wake me up.")
                    break
        while True:
...
class CS2PApp(QMainWindow):
    ...
    def speak(self, text):
        ...
        # Use a local engine for each utterance
        def _speak():
    ...
    def stop_speaking(self):
        # This will only stop the current speech, not queued ones
        self.speaking = False
        self.wake_button.setEnabled(True)
        self.input_box.setEnabled(True)
        self.stop_speaking_button.setEnabled(False)
        # Try to stop all speech engines by creating a new one and calling stop
        try:
            engine = pyttsx3.init()
            engine.stop()
        except Exception:
